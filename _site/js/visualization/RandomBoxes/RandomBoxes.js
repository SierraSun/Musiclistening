...
function preload(){
  singer=loadImage("img/71x+tqsm7gL._SL1500_.jpg");
  data=loadTable("data/test.csv","header");
}
...
function setup () {
  ...
  pixelDensity(1);
  song= loadSound("sound/soundtrack1.mp3",loaded);
  song.setVolume(0.1);
...
var hover = function onMouseOver(){
...
function draw () {
  ...
  var directlights = [
    {
      dir : [-1,-1,-2],
      col : [0.010, 0.015, 0.001],
    },
  ];

  var angle = frameCount * 0.003;
  var rad = 30;
...
// load data
  ...
  var trange = data.getRowCount()/2;
  var feq=data.get(i,1);
  var feqradius=d3.scaleLinear()
      .domain([d3.min(feq),d3.max(feq)])
      .range([3,300]);
...
function setPointlight(shader, pointlights){
  ...
  for(var i = 0; i < pointlights.length; i++){
    ...
    // transform to camera-space
    // var light_pos = m4_camera.multVec(camera.pos);
